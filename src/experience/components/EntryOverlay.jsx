...
function EntryOverlay({ onEnter, isExiting = false }) {
  ...
  return (
    <div className={`scene-entry-overlay${isExiting ? ' scene-entry-overlay--exiting' : ''}`}>
      <canvas
        ref={canvasRef}
        className="scene-entry-overlay__particles"
        aria-hidden="true"
      />
      <div className="scene-entry-overlay__content">
        <h1 className="scene-entry-overlay__title">Driftwood</h1>
        <button
          type="button"
          className="scene-entry-overlay__enter-button"
          onClick={onEnter}
        >
          Enter Driftwood
        </button>
