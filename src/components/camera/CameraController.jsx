...
function getSectionDragOffset(dragDistance) {
  const normalized = dragDistance / worldConfig.camera.dragSectionDistance
...
function CameraController() {
  ...
  useEffect(() => {
    ...
    const sectionAngle = (Math.PI * 2) / safeSectionCount
    ...
    const handlePointerMove = (event) => {
      ...
      if (stepOffset !== 0) {
        const stepDelta = stepOffset > 0 ? worldConfig.camera.dragDirection : -worldConfig.camera.dragDirection
    ...
    const unsubscribe = registerFrame(({ delta, elapsed }) => {
      ...
      orbitAngleState.current = THREE.MathUtils.damp(
        ...
        worldConfig.camera.transitionDamp,
        delta,
      )
      ...
      targetPosition.set(
        orbitCenter.x + outward.x * worldConfig.camera.orbitRadius,
        basePosition.y + breathing + smoothPointer.y * 0.2,
        orbitCenter.z + outward.z * worldConfig.camera.orbitRadius,
      )
      ...
      lookTarget.set(
        smoothedPosition.x + outward.x * worldConfig.camera.lookDistance,
        0.56 + breathing * 0.36 + smoothPointer.y * 0.2,
        smoothedPosition.z + outward.z * worldConfig.camera.lookDistance,
      )
